...
import { userColumns } from "../../teamdata";
...
const Teamdata = () => {
  ...
  const handleDelete = (id) => {
    ...
    const responseDel = fetch(`http://localhost:5000/delete-team/${id}`, {
      method: "get",
    }).then((response) => {
  ...
  const handleEdit = (id) => {
    navigate("/dashboard/team_excutive_edit/" + id);
  }
  ...
  return (
    <div className="datatable">
      <div className="datatableTitle">
        Show Team
        <Link to="/dashboard/team_excutive_add" className="link">
          Add New Team
        </Link>
      </div>
      <DataGrid
