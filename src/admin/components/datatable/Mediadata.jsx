...
import { userColumns } from "../../mediadata";
...
const Mediadata = () => {
  ...
  const handleDelete = (id) => {
    ...
    const responseDel = fetch(`http://localhost:5000/delete-media/${id}`, {
      method: "get",
    }).then((response) => {
  ...
  const handleEdit = (id) => {
    navigate("/dashboard/media_edit/" + id);
  }
 const actionColumn = [
    {
      field: "action",
  ...
  return (
    <div className="datatable">
      <div className="datatableTitle">
        Show Social Media
        <Link to="/dashboard/media_add" className="link">
          Add New Media
        </Link>
      </div>
      <DataGrid
